...
def vis_points(points, colors=None, labels=None, color_map='Paired', opacity=1.0, point_size=5.0):
    ...
    plotter = pv.Plotter()
    plotter.add_points(points, opacity=opacity, point_size=point_size, render_points_as_spheres=True, scalars=colors,
                       rgb=True)
    plotter.show()
...
def vis_multi_points(points, colors=None, labels=None,
    ...
    for i in range(n_clouds):
        ...
        if colors[i] is None and labels[i] is not None:
            ...
            if colors[i].min() < 0:
                colors[i] = np.array((colors[i] - colors[i].min) / (colors[i].max() - colors[i].min()) * 255).astype(
                    np.int8)

        plotter.add_points(points[i], opacity=opacity, point_size=point_size, render_points_as_spheres=True,
                           scalars=colors[i], rgb=True)
...
def vis_neighbors(points, neighbor_points, point_index,
    ...
    plotter.add_points(neighbor_points[point_index, :, :], point_size=neighor_point_size, color=neighbor_colors,
                       render_points_as_spheres=True)
    plotter.add_points(points[point_index], point_size=neighor_point_size * 2, color='green',
                       render_points_as_spheres=True)
    plotter.show()
