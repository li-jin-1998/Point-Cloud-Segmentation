...
def main():
    ...
    parser.add_argument('--folder', '-f', default='/mnt/algo_storage_server/PointCloudSeg/shapenet/shapenet_partseg',
                        help='Path to data folder')
    parser.add_argument('--save_ply', '-s', default=1, help='Convert .pts to .ply', action='store_true')
    ...
    for data_folder, label_folder in folders:
        print(data_folder, label_folder)
        if not os.path.exists(data_folder):
            print("{} not exist.".format(data_folder))
            continue
        ...
        for category in sorted(os.listdir(data_folder)):
            ...
            for filename in tqdm(sorted(os.listdir(data_category_folder))):
